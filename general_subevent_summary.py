...
RESOURCE_PATH = '../summarization/parsed_event_files/' ### Should contain parsed and name entity tagged files
CONCEPT_PATH = '../summarization/concept_extraction/'

def compute_summary(placefile,keyterm,date,Ts):
	...
	CLASS_INDEX = 0
        ...
        if keyterm=='Nepal':
	    CLASS = ['infrastructure','missing','shelter','volunteer']      #Nepal Earthquake
        if keyterm=='Hagupit':
            CLASS = ['infrastructure','caution','displaced','volunteer']   #Typhoon Hagupit
        if keyterm=='Pakistan':
            CLASS = ['infrastructure','missing','volunteer']               #Pakistan Flood


	for CL in CLASS:
		parsefile = RESOURCE_PATH + keyterm + '/' + CL + '_' + date + '.txt.predict'
		eventfile = RESOURCE_PATH + keyterm + '/' + CL + '_' + date + '_NER.txt'
		ifname = CONCEPT_PATH + CL + '_concept_' + date + '.txt'
		fp = codecs.open(parsefile,'r','utf-8')
		fs = codecs.open(eventfile,'r','utf-8')
		ft = codecs.open(ifname,'r','utf-8')
		print(CL,CLASS_INDEX)
		for l in fp:
			wl = l.split('\t')
			if len(wl)==8:
                        	seq = int(wl[0])
	...
	for i in range(0,index,1):
		...
		TW[i] = [v[0],v[1],mod_sub,v[3],1,v[4]]
	...
	L = len(TW.keys())
        ...
        for i in range(0,L,1):
                temp = TW[i]
                tweet_cur_window[i] = [temp[0].strip(' \t\n\r'),int(temp[3]),temp[1],temp[2],float(temp[4]),int(temp[5])] # tweet, length, content, topic

	print('Number of classes: ',CLASS_INDEX)
        ofname = keyterm + '_highsubsum_' + date + '.txt'
        optimize(tweet_cur_window,NORM_CONTENT_WEIGHT,NORM_TOPIC_WEIGHT,ofname,Ts,0.4,0.6,CLASS_INDEX)
        t1 = time.time()
...
def optimize(tweet,con_weight,sub_weight,ofname,L,A1,A2,CLASS_INDEX):
	...
	sub_word = {}
		...
		set_of_subs = v[3]
                ...
                tweet_word[tweet_index] = [v[1],set_of_words,set_of_subs,v[0],v[4],v[5]]  #Length of tweet, set of content words present in the tweet, set of subevents present in the tweet, tweet itself
                tweet_index+=1
        ...
        while CC < CLASS_INDEX:
                ...
                for i in range(0,len(sen),1):
                        ...
                        if temp[5]==CC:
                                C += sen_var[i]
                counter+=1
                m.addConstr(C,GRB.GREATER_EQUAL,2, "c%d" % (counter))
                CC+=1
...
def main():
	try:
		_, placefile, keyterm, date, Ts = sys.argv
	except Exception as e:
		print(e)
		sys.exit(0)
	compute_summary(placefile,keyterm,date,int(Ts))
	print('Done')
